...
- ILK: 누설 전류. IC에 대한 BV(Breakdown Voltage; IC가 버틸 수 있는 최대 전압 값)를 측정하고자 전압을 인가하는 과정에서 발생하는 전류. 수백nA가 나오는게 보통이다. 예를 들어 어떤 IC의 BV가 1200V라고 할 때 이에 근접한 전압 값을 인가해준다. 그 때 HO, VB, VS 핀에서 전류를 쟀을 때 나오는 값이 ILK가 된다. 