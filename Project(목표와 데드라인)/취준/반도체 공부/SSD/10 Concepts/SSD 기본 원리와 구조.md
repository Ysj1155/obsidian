...
페이지는 덮어 쓰기가 불가능하기 때문에 한번 데이터가 사용된 페이지는 반드시 삭제하는 작업을 거쳐 free상태로 전이된다. 삭제는 단일 페이지 단위로 처리될 수 없다. 그 페이지가 포함된 블록을 전체 삭제해야 한다. 삭제 명령은 컨트롤러의 공간이 필요할 때 자동적으로 내부 명령을 통해 실행된다. 