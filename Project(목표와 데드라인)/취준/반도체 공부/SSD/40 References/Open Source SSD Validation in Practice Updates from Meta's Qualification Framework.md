...
- 실행 속도 보다는 개발 속도와 코드의 가독성이 더 중요하기 때문. 하드웨어 자체의 성능 측정은 내부 엔진이 담당하고, 엔지니어는 테스트 시나리오를 빠르게 작성하고 수정하는데 집중하기 위함. 
## 하드웨어 추상화 계층(HAL)
...
스크립트의 실행 순서
	1. 발견(Discovery): 현재 서버에 꽂힌 SSD가 뭐냐
...
- 치명적 경고(Critical Warning): 온도가 너무 높거나, available space가 부족해지는 등의 신호를 정확히 보고하는가?
- 스마트 데이터: 총 쓰기 양(TBW)이나 남아있는 수명 퍼센트를 규격에 맞는 의미와 신뢰도로 보고되는지?
