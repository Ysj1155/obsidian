...
- [[SSD Mini Lab 프로젝트 허브]]
  ...
  - 핵심 산출물: [[SSD Mini Lab Portfolio Evidence]], [[External SSD Product Validation]], [[왜 평균 IOPS만 보면 안 되는가]]
- [[SSD FTL-GC White-box Validation Lab]]
  - FTL/GC/TRIM/wear/resource contention을 Python simulator로 관찰하는 white-box validation track.
  - 핵심 산출물: [[Request Timing MVP]], [[Request Timing Policy Findings]], [[Resource Contention MVP]], [[Resource Contention Quality Experiment]]
...
> SSD mini-lab에서는 fio 기반 black-box 측정으로 QD, p99/p99.9 latency, 반복 CV, sustained workload를 분석했고, FTL-GC white-box lab에서는 GC policy가 WAF, wear, GC pause, resource contention에 미치는 영향을 simulator trace로 검증했습니다. 두 track을 분리해 실제 장치 관찰 결과와 내부 모델 기반 해석의 한계를 구분했습니다.
