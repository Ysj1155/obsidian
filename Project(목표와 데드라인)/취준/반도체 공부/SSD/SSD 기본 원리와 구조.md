...
### 데이터 쓰기
새로운 데이터를 기록할 때 기존의 데이터를 삭제하고 빈 셀에 데이터를 기록한다. 이 과정에서 컨트롤러가 데이터를 최적화된 위치에 기록할 수 있도록 한다. 쓰기를 실행할 때에도 페이지 단위로 하나 또는 여러개 단위의 페이지로 쓰기가 실행된다. 단 하나의 바이트만 기록하는 경우에도 반드시 전체 페이지가 기록되어야 한다. 
페이지에 