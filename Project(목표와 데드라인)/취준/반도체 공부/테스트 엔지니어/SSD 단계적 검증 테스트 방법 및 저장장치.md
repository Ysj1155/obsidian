...
- SSD 용량이 커질수록(엔터프라이즈 SSD 16TB/32TB/64TB급) preconditioning 시간이 길어지고 테스트가 시간/장비 점유 측면에서 비싸진다. 
...
-> Full capacity 기준의 정규 성능 검증 방식:
전체 사용자 용량(full LBA range)을 대상으로 sanitize/erase 후 preconditioning(대용량 순차/랜덤 쓰기 반복) 수행해서 steady-state 도달 후 성능 측정(예: 4K random write). 그리고 펌웨어/기능 수정 버전이 나올 때마다 이 과정을 반복. 

기존 방식은 정확하지만 느리다. 개발 단계용 가속 방법이 필요하다는게 이 특허의 문제 정의. 
### 특허의 핵심 아이디어
...
원래는 16TB 전체를 계속 예열/테스트해야 했다면 특허 방식은 예를 들어 4TB 수준의 short-stroked 사용자 용량이 전체 용량의 steady-state 랜덤 쓰기 성능과 유사하게 맞아떨어지는 지점을 찾고, 그 이후부터는 그 용량으로만 빠르게 반복 검증하는 방식이다.
이게 빠른 이유는 preconditioning 시간이 사용자 용량에 비례해서 크게 줄어들기 때문이다.

왜 사용자 용량을 줄이면 성능이 변할까? 물리적 낸드 플래시 용량은 그대로인데 Short Stroke로 사용자 용량(LBA 범위)을 줄이면, 상대적으로 OP 영역이 커지는 효과가 발생한다. 

OP가 커지면 쓰기 증폭(WAF)이 낮아져 성능이 올라가고, OP가 작으면(Full Cap) GC 부하로 성능이 내려간다. 이 특허는 사용자 용량을 적절히 조절하여, 늘어난 OP로 인한 성능 이득과 Full Capacity의 Steady-state 성능이 교차하는 지점을 찾는 것이 핵심이다.
...
4. 일치했다고 끝내지 않고, **여러 번 반복 측정(예: 6회)** 해서 일관되게 맞는지 확인 후 확정.
	 명세서에서는 단순히 일치하면 끝내는 것이 아니라 **6회 이상 반복 측정**하여 결과의 편차(Standard Deviation)를 확인하라고 되어있다. 이는 검증(Validation)에서 매우 중요한 재현성(Reproducibility)을 확보하기 위한 장치다.
...
### 직무 활동 해볼거
- FIO 스크립트 자동화: 명세서에 나온 Preconditioning 및 측정 조건을 바탕으로 `fio` 스크립트를 짜보고, 파이썬 등으로 성능을 비교하여 용량을 자동으로 조절(Binary Search 방식 등)하는 캘리브레이션 툴의 로직을 설계해 보는 경험.
- JESD218B 규격 비교: 특허에서 언급된 JEDEC 표준의 Short Stroke 개념과 실제 이 특허의 방식이 어떻게 다른지(표준은 외삽법(Extrapolation) 위주, 특허는 성능 매칭 위주) 비교 분석.
