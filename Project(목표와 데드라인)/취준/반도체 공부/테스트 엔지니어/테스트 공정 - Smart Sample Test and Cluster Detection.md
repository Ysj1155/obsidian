...
- SST(Smart Sample Test)
	...
	위 MAP에서 Good Die는 전부 초록색이다. 다른 색깔이 칠해진 것은 어떤 불량 이슈인지 색깔별로 분류해서 보여준다. 이슈는 다양하다. 전기적 특성 문제면 Bin 1 ~N번으로 분류되고 동작은 정상이나 외관에서 문제가 발생한거면 WDM으로 분류된다. 그 외에도 뭐가 많다고 하니 조사 추가 바람.

- Cluster Detection
	![[Pasted image 20260127220758.png]]
	각 다른 색깔이 각자 다른 불량 이슈라고 정의. 빨간색 사이사이 컬러링 되지 않은 부분은 Good Die라고 판단할 수 있다. 하지만 그 주변에 분류되지 않은 잠재적 불량 이슈 가능성이 높기 때문에 저렇게 웨이퍼의 어느 한 부분에서 불량이 탄집군처럼 터져있을 때는 그냥 그 영역 전체를 불량으로 처리하는 기법이다. 